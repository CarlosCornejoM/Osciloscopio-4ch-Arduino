...
public void setup() {
  ...
  resetEixos=new Botao("Ejes",marg1+50,tela.y,45,20);
  resetMedir=new Botao("Medidas",resetEixos.x+resetEixos.w+2,tela.y,60,20);
  ...
  verPontos=new CheckBox("Datos experimentales", chXYZ.x, chXYZ.y+chXYZ.h+5, 15);
  calcFreq=new CheckBox("Frecuencia", verPontos.x, verPontos.y+verPontos.h+5, 15);
  grafDif=new CheckBox("Derivada",calcFreq.x+calcFreq.w-3,calcFreq.y,15);
  ...
  save=new Botao("Guardar Datos ",calcFreq.x,calcFreq.y+calcFreq.h+5,150,20);


  //medidor de resistor/capacitor
  pnlRC=new Painel("", tela.x, tela.y+tela.h+10, 0, 0);
  ...
  pnlSinal=new Painel("", pnlRC.x+pnlRC.w+10, pnlRC.y, 180, 85);
  sinal=new CheckBox("Generar Señales", pnlSinal.x, pnlSinal.y, 15);
  ...
  tonSinal=new Dial(escLinear, altSolta, nInt, !fmt, "Duty", "%", 25f, 0f, 100f, tSinal.x, tSinal.y+tSinal.h+2, tSinal.w, tSinal.h);

  // posicionamento da Amostragem 
  pnlAmostra=new Painel("Muestreo", pnlSinal.x+pnlSinal.w+10, pnlSinal.y, 250, 85);
  dt=new Dial(escLog, altSolta, nInt, fmt, "dt", "s", 1e-3f, 10e-6f, 2f, pnlAmostra.x+5, pnlAmostra.y+20, 170, 20);
  dtReal=new FmtNum(0,nInt,fmt);
  q=new Dial(escLinear, altSolta, nInt, !fmt, "N", "", 100, 1, 100, dt.x+dt.w+5, dt.y, 60, 20);
  tTotal=new FmtNum(dt.v.getV()*q.v.getV(), !nInt);
  tTotalReal=new FmtNum(0,!nInt);
  umaAmostra=new Botao("Único", dt.x, dt.y+dt.h+5, 60, 20);
  variasAmostras=new Botao("Múltiple", umaAmostra.x+umaAmostra.w+5, umaAmostra.y, umaAmostra.w, umaAmostra.h);
  fluxoContinuo=new Botao("Continuo", variasAmostras.x+variasAmostras.w+5, variasAmostras.y, variasAmostras.w, variasAmostras.h);
...
public void draw() {
  ...
  textAlign(LEFT, TOP);
  textSize(20); 
  text("Osciloscopio 4 canales ", tela.x-10, 12);
  fill(0); 
  textSize(16); 
  text(" 5V-3kHz MAX, PWM y SW, ~9~10", tela.x-15, tela.y-20);
  tela.display();
  // Bot\u00f5es de demonstra\u00e7\u00e3o
  textSize(15);
  fill(0); 
  textAlign(RIGHT, CENTER); 
  text("Simular", demo[0].x-5, demo[0].y+demo[0].h/2); //tela.x+tela.w+10,tela.y);
  text("Resetear",resetEixos.x-5,resetEixos.y+resetEixos.h/2);
  //text("Medir dt,dV", medir[0].x-5,medir[0].y+medir[0].h/2);
  //text("Fijar", trigger[0].x-5,trigger[0].y+trigger[0].h/2);
  chXYZ.display();
  //XYZ.display();
  //for (byte k=0; k<3;k++){
  //  selXYZ[k].display();
  ...
  if (save.mouseClicado()){
        ...
        if (fluxoContinuo.clicado){
          if (outputOpen==false){ // n\u00e3o est\u00e1 gravando, ent\u00e3o iniciar a grava\u00e7\u00e3o
            ...
            String fileName =nf(day(),2)+"-"+nf(month(),2)+"-"+nf(year(),4)+" "+"t0="+nf(hour(),2)+";"+nf(minute(),2)+";"+nf(second(),2)+".txt";
        ...
        if (outputOpen) {
          ...
          //for (int k2=0; k2<q.v.v;k2++){
            ...
            if (qSave % 100==0) { // de 100 em 100
              save.tex="N "+nf(qSave)+ " , Click para detener";
              output.flush();
            }
          //}
        }
...
class Canal{
 ...
 Canal(byte n_,int nCor_, int x_, int y_, int w_, int h_){
     n=n_; nCor=nCor_;
     x=x_; y=y_; w=w_; h=h_;
     //chN=new Botao("A"+str(n),x,y,w/2,20,nCor,nCor);
     chN=new Botao("A"+str(n),x,y,w/2,15,nCor,nCor);
     //inv=new CheckBox("INV",x+w/2+8,y+4,12);
     trigger=new CheckBox("Fijar ",x+w/2+3,y+3,14);
     chN.clicado=true;
     ...
     curva=new CheckBox("Ajuste ",ft.x+ft.w/2,ft.y+ft.h+5,15);
 }
...
class CanalXYZ{
  ...
  CanalXYZ(int nCor_,int x_, int y_, int w_, int h_){
     ...
     verCanais=new CheckBox("Mostrar Canales",curvaSuave.x,curvaSuave.y+curvaSuave.h+2,15);
...
class Com{
   Serial port;
   String ports[]=append(Serial.list(),"Seleccionar puerto Serial");
   String portName;
   int indPort=ports.length-1;
   //String speed="115200";
   //String speed="250000";
   String speeds[]={"9600","115200","250000","Velocidad COM"};
   //int portSpeed;
   ...
   Com(Serial portt,int xt, int yt, int wt, int ht){
      ...
      refresh=new TextBox("Actualizar",CENTER,PApplet.parseInt(x+0.7f*w),y,PApplet.parseInt(0.3f*w),dh);
      //line 2
      selectSerial=new TextBox("Elegir Puerto Serial",CENTER,x,y+h/3,w,dh);
      //line 3
      selectSpeed=new TextBox("Velocidad COM",CENTER,x,y+2*h/3,PApplet.parseInt(0.4f*w),dh);
      versionArduino=new TextBox("",CENTER,PApplet.parseInt(x+0.8f*w)-4,PApplet.parseInt(y+2*h/3),PApplet.parseInt(0.2f*w)+4,dh);
      onOff=new TextBox("Encender",CENTER,PApplet.parseInt(x+0.4f*w),y+2*h/3,-4+PApplet.parseInt(0.4f*w),dh);
      //onOff=new TextBox("off",CENTER,int(x+0.7*w),y,int(0.3*w),dh);
 } 
   public void display(){
     strokeWeight(1); stroke(0);fill(250);
     onOff.tex="Encender";
     if (conectado) {
         cor=color(0,255,0);
         onOff.tex="Apagar";
     ...
     //if (mouseY>y && mouseY<y+h){
       if (refresh.mouseClicado()) { // recarregar a lista das COMs
         if (!conectado) {  // not connected
            ports=append(Serial.list(),"Seleccione serial");
            if (ports.length>1){
